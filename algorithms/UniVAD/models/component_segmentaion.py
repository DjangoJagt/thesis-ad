...
def grounding_segmentation(img_paths,save_path,config):
    ...
    sam_checkpoint = config.get('sam_checkpoint', './pretrained_ckpts/sam_hq_vit_h.pth')

    # Prefer HQ registry if variant available; fall back to regular registry
    if sam_variant in sam_hq_model_registry:
        sam_ctor = sam_hq_model_registry[sam_variant]
        registry_type = "HQ"
    elif sam_variant in sam_model_registry:
        sam_ctor = sam_model_registry[sam_variant]
        registry_type = "Standard"
    else:
        raise ValueError(f"Unknown sam_variant '{sam_variant}'. Supported: {list(sam_hq_model_registry.keys()) + list(sam_model_registry.keys())}")

    print(f"🔧 Loading SAM model: {registry_type} Registry | Variant: {sam_variant} | Checkpoint: {sam_checkpoint}")
    sam = sam_ctor(sam_checkpoint).to(device)
...
class ClassHistogram():
    ...
    def cluster_component_features(self,feature_list=['area','color','position','cnn_image']):
        ...
        if self.config['reduce_dim']:
            ...
            self.all_component_feats = self.projector(torch.from_numpy(self.all_component_feats).type(torch.float32).to(device))
