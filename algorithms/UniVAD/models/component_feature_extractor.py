...
class ComponentFeatureExtractor():
    ...
    def compute_component_feature(self,image,masks):
        ...
        for i,mask in enumerate(masks):
            ...
            feature['area'] = torch.tensor(np.array([area])/mask.size).to(device)
            ...
            feature['color'] = torch.tensor(np.array([color_value])).to(device)
            ...
            feature['position'] = torch.tensor(position).to(device)
        ...
        if self.clip_model is not None and self.dino_model is not None:
            ...
            with torch.no_grad():
                ...
                self.config["transform_clip"](rotated_images.to(device)), self.out_layers)

                dino_patch_tokens = self.dino_model.forward_features(
                # self.config["transform_dino"](rotated_images.cuda()))["x_norm_patchtokens"]
                self.config["transform_dino"](rotated_images.to(device)))["x_norm_patchtokens"]
