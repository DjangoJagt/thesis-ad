...
def preview_sample_images(base_dir, class_dirs=DEFAULT_CLASS_DIRS, subpath=DEFAULT_SUBPATH, 
    ...
    output_dir = Path(output_dir or base_dir / "hough_masks_2")
    output_dir.mkdir(parents=True, exist_ok=True)

    for class_dir in class_dirs:
        # Process all images in train/good, train/issue, test/good, test/issue
        for split in ["train", "test"]:
            for category in ["good", "issue"]:
                folder_path = base_dir / class_dir / split / category
                ...
                for image_path in sorted(folder_path.glob("*")):
                    ...
                    print(f"Processing: {class_dir}/{split}/{category}/{image_path.name}")
                    image = cv2.imread(str(image_path))
                    if image is None: continue

                    masked, debug = robust_industrial_crop(image, wall_ratio=wall_ratio)
                    
                    # Save with descriptive name including class, split, and category
                    out_name = f"{class_dir}_{split}_{category}_{image_path.stem}"
                    cv2.imwrite(str(output_dir / f"{out_name}_masked.png"), masked)
                    cv2.imwrite(str(output_dir / f"{out_name}_debug.jpg"), debug)
...
def main():
    ...
    preview_sample_images(base_dir=args.base_dir, wall_ratio=args.wall_ratio)
