...
def preview_sample_images(base_dir, class_dirs=DEFAULT_CLASS_DIRS, subpath=DEFAULT_SUBPATH, 
    ...
    output_dir = Path(output_dir or base_dir / "cropped_previews")
    ...
    for class_dir in class_dirs:
        ...
        cv2.imwrite(str(output_dir / f"{class_dir}_{image_path.stem}_masked.png"), masked)
        cv2.imwrite(str(output_dir / f"{class_dir}_{image_path.stem}_debug.jpg"), debug)
        print(f"[OK] Saved: {output_dir / f'{class_dir}_{image_path.stem}_debug.jpg'}")
