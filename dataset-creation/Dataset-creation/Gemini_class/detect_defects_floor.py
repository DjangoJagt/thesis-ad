...
from Sick_dataset.utils.crop import robust_industrial_crop, extract_floor_tiles
...
- Image 2A is the Tote Floor (Top Tile) provided ONLY to detect leakage/wetness.
- Image 2B is the Tote Floor (Bottom Tile) provided ONLY to detect leakage/wetness.
- Do NOT use Images 2A/2B to evaluate SKU correctness, packaging integrity (except wetness), quantity, or other anomalies.
- If wetness, pooling liquid, streaks, or fresh leakage is visible in either tile, classify as ANOMALOUS.
...
def get_image_data_floor(
    ...
    """
    Extracts tote floor tiles from the processed tote image and returns
    ...
    try:
        tile_top, tile_bottom = extract_floor_tiles(cropped_img)
    except Exception as e:
        LOGGER.warning(
    ...
    tile_top_b64, tile_top_mime = encode_tile(tile_top)
    tile_bottom_b64, tile_bottom_mime = encode_tile(tile_bottom)

    return tile_top_b64, tile_top_mime, tile_bottom_b64, tile_bottom_mime
...
async def analyze_row(
    ...
    async with semaphore: # Wait here if 5 requests are already running
        ...
        try:
            ...
            # Optionally append floor tiles for leakage-only assessment
            if tile_top_b64 and tile_top_mime:
                content_items.extend([
                    {"type": "text", "text": "Image 2A: Tote Floor (Top Tile) — Leakage-only"},
                    {"type": "image_url", "image_url": {"url": f"data:{tile_top_mime};base64,{tile_top_b64}"}},
                ])
            if tile_bottom_b64 and tile_bottom_mime:
                content_items.extend([
                    {"type": "text", "text": "Image 2B: Tote Floor (Bottom Tile) — Leakage-only"},
                    {"type": "image_url", "image_url": {"url": f"data:{tile_bottom_mime};base64,{tile_bottom_b64}"}},
                ])
