...
DEFAULT_CLASS_DIRS = ["10512371", "11299303", "11463862", "11478313", "11738660", "11780914", "11801970", "11810623", "12077488", "12461215"]
...
def robust_iphone_crop(image, wall_ratio=0.06):
    ...
    top_w_pct = 0.35      # Search outer 35% at the top (Wide)
    bot_w_pct = 0.12      # Search outer 15% at the bottom (Narrow to avoid products)
    ...
    # # Top/Bottom ROIs (Keep these rectangular for now, they usually work fine)
    # roi_h_top = int(H * 0.32)    
    # roi_h_bottom = int(H * 0.20) 

    # ---------------------------------------------------------
    # 1. DEFINE ROI LIMITS
    # ---------------------------------------------------------
    # Top ROI: NOW A STRIP from 15% to 32%
    roi_h_top_start = int(H * 0.15)  # Skip the top 15% (background)
    roi_h_top_end   = int(H * 0.33)  # Stop searching at 32%

    # Bottom ROI: Standard 20% from the bottom
    roi_h_bottom = int(H * 0.20)
    ...
    # cv2.rectangle(debug_img, (0, 0), (W, roi_h_top), (255, 0, 0), 2)
    # Draw top box starting from 'start' instead of 0
    cv2.rectangle(debug_img, (0, roi_h_top_start), (W, roi_h_top_end), (255, 0, 0), 2)
    cv2.rectangle(debug_img, (0, H-roi_h_bottom), (W, H), (255, 0, 0), 2)
    ...
    # For Top/Bottom, simple slicing is still faster/easier
    t_edges_masked = edges.copy()
    # t_edges_masked[roi_h_top:, :] = 0  # Mask out bottom part
    t_edges_masked[roi_h_top_end:, :] = 0    # Mask everything BELOW the strip
    t_edges_masked[:roi_h_top_start, :] = 0  # Mask everything ABOVE the strip (NEW)
    
    b_edges_masked = edges.copy()
    b_edges_masked[:H-roi_h_bottom, :] = 0 # Mask out top part

    def find_lines(masked_edges, threshold, min_len, max_gap = 60):
    ...
    def get_best_vertical(lines, is_left, offset_x=0): # offset_x not really needed for Poly but kept for compat
        ...
        for line in lines:
            ...
            if is_left:
                ...
                if 90 < angle < 120: valid_angle = True
            else:
                # Right Wall: Expect / (85-110 deg)
                if 60 < angle < 90: valid_angle = True
    ...
    # HORIZONTAL LOGIC
    # ---------------------------------------------------------
    def get_best_horizontal(lines, is_top):
        if lines is None: return None
        best_line = None
        best_y_score = -1 if is_top else 99999 #-1
        
        for line in lines:
            x1, y1, x2, y2 = line[0]
            angle = abs(math.atan2(y2 - y1, x2 - x1) * 180.0 / np.pi)
            
            if angle < 1.5 or angle > 178.5: 
                avg_y = (y1 + y2) / 2
                cv2.line(debug_img, (x1, y1), (x2, y2), (255, 255, 0), 1)
                if is_top: 
                    if avg_y > best_y_score:
                        best_y_score = avg_y
                        best_line = (x1, y1, x2, y2)
                else:
                    if avg_y < best_y_score:
    ...
    # EXECUTE SEARCH
    l_candidates = find_lines(l_edges_masked, threshold=50, min_len=H//3, max_gap=55)
    r_candidates = find_lines(r_edges_masked, threshold=50, min_len=H//3, max_gap=55)
    t_candidates = find_lines(t_edges_masked, threshold=50, min_len=W//3, max_gap=30 )
    b_candidates = find_lines(b_edges_masked, threshold=50, min_len=W//2.8, max_gap=25)

    l_line = get_best_vertical(l_candidates, True)
    r_line = get_best_vertical(r_candidates, False)
    t_line = get_best_horizontal(t_candidates, True)
    b_line = get_best_horizontal(b_candidates, False)
    ...
    def extrapolate_horizontal(line):
        if line is None: return None
        x1, y1, x2, y2 = line
    ...
    t_pts = extrapolate_horizontal(t_line) or ((0, 0), (W, 0))
    b_pts = extrapolate_horizontal(b_line) or ((0, H), (W, H))
    
    # Draw Green Final Lines
    for pts in [l_pts, r_pts, t_pts, b_pts]:
    ...
    cv2.fillPoly(masked_image, [pts], (0,0,0))
    # Top
    pts = np.array([[0,0], t_pts[0], t_pts[1], [W,0]])
    cv2.fillPoly(masked_image, [pts], (0,0,0))
    # Bottom
    pts = np.array([b_pts[0], [0,H], [W,H], b_pts[1]])
    cv2.fillPoly(masked_image, [pts], (0,0,0))
...
def main():
    ...
    process_all_images(base_dir=args.base_dir)
...
#     parser.add_argument("--base-dir", type=Path, default=Path(".")) 
#     args = parser.parse_args()
#     preview_sample_images(base_dir=args.base_dir)
